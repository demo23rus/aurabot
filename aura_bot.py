...
def main_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🧠 AI-Психолог", callback_data="psycho"),
         InlineKeyboardButton(text="📔 Личный дневник", callback_data="diary")],
...
DIARY_SYSTEM = """Ты — тихий хранитель дневника. Человек записывает свои мысли голосом.

Твоя единственная задача — принять запись тепло и коротко.
Никаких советов. Никакого анализа. Никакой психологии. Никаких выводов о паттернах.

Просто скажи одним-двумя предложениями что услышал — как будто кивнул головой.
Потом задай один простой тёплый вопрос для следующей записи.

Пример хорошего ответа:
"Сегодня был насыщенный день — и трудный, и со светлым финалом. Что из сегодняшнего хочется запомнить?"

Пишешь только на русском. Не начинай с Конечно, Отлично, Вот, Готово. Обращайся на ты.
...
📔 Личный дневник — храни мысли голосом
...
async def tariffs(callback: CallbackQuery):
    ...
    await callback.message.answer(
        ...
        f"Совместимость, Личный дневник — безлимит\n"
...
async def diary_handler(callback: CallbackQuery):
    ...
    await callback.message.answer(
        ...
        "— Ты записываешь голосовые сообщения\n"
        "— Бот тихо принимает каждую запись и хранит её\n"
        "— Все записи сохраняются — ты можешь возвращаться к ним\n"
        "— Бот не даёт советов — просто слушает и задаёт один вопрос\n\n"
        "Для работы с проблемами и советов — используй 🧠 AI-Психолог\n\n"
...
async def handle_voice(message: Message):
    ...
    try:
        ...
        # Дневник — проверяем доступ
        if step == "diary":
            access = await check_access(user_id, "diary")
            if access not in ("ok", "pro"):
                set_step(user_id, "idle")
                await message.answer(
                    "📔 Личный дневник доступен с тарифа 🟢 Старт.\n\nЗдесь хранятся все твои записи и разборы.\n\n190 руб/мес — открой доступ 👇",
                    reply_markup=upgrade_menu()
                )
                return
