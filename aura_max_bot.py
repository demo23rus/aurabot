...
def init_db():
    ...
    if "chat_id" not in user_columns:
        c.execute("ALTER TABLE users ADD COLUMN chat_id INTEGER DEFAULT 0")
...
def get_user(user_id, username="", first_name="", chat_id=None):
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    c.execute("INSERT OR IGNORE INTO users (user_id, username, first_name, chat_id, registered_at) VALUES (?,?,?,?,?)",
              (user_id, username, first_name, int(chat_id or 0), datetime.now(MOSCOW).isoformat()))
    c.execute("INSERT OR IGNORE INTO limits (user_id) VALUES (?)", (user_id,))
    updates=[]; values=[]
    if username:
        updates.append("username=?"); values.append(username)
    if first_name:
        updates.append("first_name=?"); values.append(first_name)
    if chat_id:
        updates.append("chat_id=?"); values.append(int(chat_id))
    if updates:
        values.append(user_id)
        c.execute(f"UPDATE users SET {', '.join(updates)} WHERE user_id=?", values)
    conn.commit()
    c.execute("SELECT step, birth_date, chat_id FROM users WHERE user_id=?", (user_id,))
    row = c.fetchone()
    conn.close()
    return {"step": row[0], "birth_date": row[1], "chat_id": int(row[2] or 0)}


def remember_user_chat(user_id, chat_id, username="", first_name=""):
    if not user_id:
        return
    get_user(user_id, username, first_name, chat_id=chat_id)
    with db_connect() as conn:
        conn.execute("INSERT OR IGNORE INTO user_profiles (user_id) VALUES (?)", (user_id,))
        conn.execute("UPDATE user_profiles SET stopped=0 WHERE user_id=?", (user_id,))
...
async def send_user_message(user_id, text, buttons=None):
    """Send a background notification using the stored MAX dialog chat_id.

    A user_id fallback is kept for legacy users who have not opened the updated bot yet.
    The next interaction stores their real chat_id automatically.
    """
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        response = await client.post(
            f"{MAX_API}/messages?user_id={int(user_id)}&disable_link_preview=true",
            json=payload,
            headers=headers,
        )
    if response.status_code >= 400:
        raise RuntimeError(f"MAX background message error {response.status_code}: {response.text[:300]}")
    try:
        return response.json()
    except Exception:
        return {"ok": True}
        if response.status_code >= 400:
            logging.error("MAX send_user_message user_id=%s status=%s body=%s", user_id, response.status_code, response.text[:1000])
...
CHIROMANCY_SYSTEM = "Ты делаешь развлекательный символический разбор линий ладони. Не определяй здоровье, срок жизни или судьбу как факт. Формулируй как повод для саморефлексии. Только русский, без Markdown."
PHYSIO_SYSTEM = "Ты делаешь только развлекательный и рефлексивный разбор визуального впечатления от фото. Не определяй характер как факт, здоровье, интеллект, этничность, религию, ориентацию, надёжность или диагнозы. Используй формулировки может создавать впечатление. Только русский, без Markdown."
GRAPHO_SYSTEM = "Ты делаешь развлекательный рефлексивный разбор особенностей почерка. Не выдавай выводы о личности, интеллекте или здоровье за факты. Только русский, без Markdown."
...
COMPAT_PHOTO_SYSTEM = "Ты делаешь развлекательную рефлексию о визуальной динамике пары по фото. Не утверждай совместимость как факт и не определяй чувствительные черты. Дай вопросы для разговора и подчеркни, что отношения определяются поведением и общением. Только русский, без Markdown."
            ...
            for payment_id, user_id, plan in get_pending_payments():
                try:
                    ...
                    if status == "succeeded":
                        ...
                        delete_pending_payment(payment_id)
                        await send_user_message(user_id, "❌ Платёж отменён. Попробуй снова.", main_menu_buttons())
                except Exception as e:
    ...
    while True:
            ...
            with db_connect() as conn:
                users = conn.execute(
                    """SELECT u.user_id FROM users u
                       ...
                       WHERE COALESCE(p.stopped,0)=0 AND COALESCE(u.chat_id,0)<>0"""
                ).fetchall()
            for (uid,) in users:
                try:
                    await send_user_message(uid, f"{heading}\n\n{clean_display_text(lunar_text)}")
                    await asyncio.sleep(0.05)
                except Exception as exc:
                    logging.warning("daily lunar MAX %s: %s", uid, exc)
        except Exception as exc:
            logging.exception("Ошибка общего утреннего сообщения MAX: %s", exc)
            ...
            pro_users = conn.execute(
                """SELECT u.user_id, COALESCE(p.birth_date,u.birth_date)
                   ...
                   WHERE s.plan='aura_pro' AND s.sub_end>?
                     ...
                     AND COALESCE(p.stopped,0)=0 AND COALESCE(u.chat_id,0)<>0""",
        ...
        for uid, birth in pro_users:
            try:
                ...
                await send_user_message(uid, f"⭐️ Твоя личная подсказка на {today}\n\n{clean_display_text(text)}")
                await asyncio.sleep(0.05)
            except Exception as exc:
                logging.warning("personal daily MAX %s: %s", uid, exc)
...
def build_admin_funnel_report():
    ...
    with db_connect() as conn:
        channel_entries_7d = conn.execute(
            "SELECT COUNT(*) FROM channel_clicks WHERE clicked_at>=?",
            (since7,),
        ).fetchone()[0]
        top_targets = conn.execute(
            "SELECT target, COUNT(*) FROM channel_clicks WHERE clicked_at>=? GROUP BY target ORDER BY COUNT(*) DESC LIMIT 6",
            (since7,),
        ).fetchall()
    ...
    top_lines = [f"• {target or 'unknown'} — {cnt}" for target, cnt in top_targets] or ["• пока нет переходов"]
...
async def process_command(chat_id, user_id, text, username="", first_name=""):
    ...
    if text == "/reviews":
        ...
        if not rows:
            await send_message(chat_id, "Пока нет сохранённых отзывов.")
            return
        listing = ["⭐️ Последние отзывы", "", "Публикуй только после разрешения автора: /publish_review ID"]
        for review_id, first, review, created in rows:
            listing.append(
                f"\nID {review_id} • {first or 'Анонимно'} • {created[:10]}\n"
                f"{truncate_at_sentence(review, 260)}"
            )
        await send_message(chat_id, "\n".join(listing))
        return

    if text.startswith("/publish_review"):
        if not is_owner(user_id, username):
            await send_message(chat_id, "⛔ Команда доступна владельцу.")
            return
        parts = text.split()
        if len(parts) != 2 or not parts[1].isdigit():
            await send_message(chat_id, "Формат: /publish_review ID\nПубликуй только после разрешения автора.")
            return
        ok = await publish_saved_review(int(parts[1]))
        await send_message(
            chat_id,
            "✅ Анонимный отзыв опубликован в канале."
            if ok else "❌ Отзыв не найден или публикация не удалась.",
        )
        return
    ...
    if text == "/stats":
        if not is_owner(user_id, username):
            await send_message(chat_id, "⛔ Команда доступна владельцу.")
            return
        await send_message(chat_id, build_admin_stats_report(), main_menu_buttons())
        return
    ...
    if step == "my_day_birth":
        ...
        set_step(user_id, "idle")
        try:
            values = calculate_numerology_data(birth_dt, datetime.now(MOSCOW))
            result = await generate_text(
                HOROSCOPE_SYSTEM,
                f"Дата рождения: {birth}. Сегодня {datetime.now(MOSCOW).strftime('%d.%m.%Y')}. "
                f"Расчёт личного дня: {values['personal_day']}. Дай персональную подсказку: энергия дня, "
                "отношения, деньги, главное действие и вечерняя практика. Не обещай неизбежных событий.",
            )
        except Exception as exc:
    ...
    if step == "psycho":
        if looks_like_crisis(text):
            ...
            await notify_owner("🆘 Кризисное сообщение", user_id, "psycho", text)
            return
...
async def startup():
    ...
    logging.info("Aura MAX Bot Premium 10/10 запущен")
...
async def webhook(request: Request):
    try:
        ...
        if update_type == "bot_started":
            ...
            remember_user_chat(user_id, chat_id, username, first_name)
            remember_owner_chat(user_id, chat_id)
            with sqlite3.connect(DB) as conn:
                conn.execute("INSERT OR IGNORE INTO user_profiles (user_id) VALUES (?)", (user_id,))
            ...
            if target:
                ...
                entry_copy = {
                    ...
                    "channel_money": "💰 Расскажи, что сейчас происходит с деньгами, какой результат ты хочешь и что, по твоему ощущению, мешает.",
                    ...
                    "channel_diary": "📝 Сделаем бесплатную вечернюю рефлексию. Напиши, что забрало силы, что получилось и что не хочется нести в завтра.",
            ...
            if chat_id and user_id:
                remember_user_chat(user_id, chat_id, "", first_name)
                remember_owner_chat(user_id, chat_id)
            ...
            if user_id:
                with db_connect() as conn:
                    conn.execute("INSERT OR IGNORE INTO user_profiles (user_id) VALUES (?)", (user_id,))
                    conn.execute("UPDATE user_profiles SET stopped=1 WHERE user_id=?", (user_id,))
...
<body>
  ...
  <p>Возвращайся в AuraMAX: подписка или разовый разбор появятся автоматически после подтверждения платежа.</p>
  <p>Бот → <strong>Аура — Психология</strong></p>
</body>
</html>"""
...
async def send_to_channel(text, button_text, start_payload, image_path=None):
    ...
    if image_path and os.path.exists(str(image_path)):
        try:
            ...
            logging.warning("Канал MAX: визуал не отправлен: %s", exc)
    attachments.append({
        "type": "inline_keyboard",
        "payload": {"buttons": native_channel_button(button_text, start_payload)},
    })
    payload = {"text": truncate_at_sentence(text, 3900), "attachments": attachments}
    async with httpx.AsyncClient(timeout=30) as client:
        response = await client.post(
    ...
    if response.status_code >= 400:
        raise RuntimeError(f"MAX channel error {response.status_code}: {response.text[:500]}")
    try:
        return response.json()
    except Exception:
        return {"ok": True}
...
async def publish_channel_slot(dt, rubric):
    ...
    try:
        ...
        await notify_owner("⚠️ Не вышел пост в MAX-канале", 0, rubric, f"{key}: {exc}")
        return False
...
Когда общего поста недостаточно, AuraMAX поможет разобрать именно твою ситуацию.
...
async def channel_posting_loop():
    ...
    while True:
        ...
        try:
            ...
            logging.exception("Channel loop MAX: %s", exc)
